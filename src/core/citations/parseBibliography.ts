...
export function parseBibliography(pages: PageText[]): BibliographyParseResult | null {
  ...
  const bibLines = allLines.slice(headingIdx + 1, stopIdx).filter((l) => l.text.trim().length > 0);
...
function groupLinesIntoEntries(lines: PageLine[]): PageLine[][] {
  ...
  for (const line of lines) {
    ...
    if (numberedStart) {
      ...
      const isColumnOrPageBreak =
        line.page !== prev.page ||
        line.y > prev.y + COLUMN_Y_JUMP ||
        Math.abs(line.x - prev.x) > COLUMN_X_JUMP;
      isNewEntry = !isColumnOrPageBreak && line.x < prev.x - OUTDENT_EPS;
    }
